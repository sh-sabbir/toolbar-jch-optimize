jQuery(document).ready(function () {
	var percentage = jQuery("#wp-admin-bar-jch-helper-cache-info .jch-helper-radial-bar").attr(
		"percentage"
	);
	var rotate = percentage * 1.8;

	jQuery(
		"#wp-admin-bar-jch-helper-cache-info .jch-helper-radial-bar .mask.full, #wp-admin-bar-jch-helper-cache-info .jch-helper-radial-bar .fill"
	).css({
	...
	jQuery("#wp-admin-bar-jch-helper-cache-info .jch-helper-radial-bar .inset").css(
		"background-color",
		jQuery("#wp-admin-bar-autoptimize .ab-sub-wrapper").css("background-color")
	);
	jQuery("#wp-admin-bar-jch-helper-delete-cache .ab-item").css(
		"background-color",
		jQuery("#wpadminbar").css("background-color")
	);

	jQuery("#wp-admin-bar-jch-helper-default li").click(function (e) {
		var id =
		...
		if (id == "wp-admin-bar-jch-helper-delete-cache") {
			action = "jch_helper_delete_cache";
		} else {
			return;
		}
		...
		var modal_loading = jQuery('<div class="jch-helper-loading"></div>')
			.appendTo("body")
			.show();

		var success = function () {
			// Reset output values & class names of cache info
			jQuery("#wp-admin-bar-jch-helper-cache-info .size")
				.attr("class", "size green")
				.html("0.00 B");
			jQuery("#wp-admin-bar-jch-helper-cache-info .files").html("0");
			jQuery("#wp-admin-bar-jch-helper-cache-info .percentage .numbers")
				.attr("class", "numbers green")
				.html("0%");
			jQuery("#wp-admin-bar-jch-helper-cache-info .jch-helper-radial-bar .fill").attr(
				"class",
				"fill bg-green"
			);
		...
		jQuery.ajax({
			type: "GET",
			url: jch_helper_ajax_object.ajaxurl,
			data: { action: action, nonce: jch_helper_ajax_object.nonce },
			dataType: "json",
			cache: false,
			timeout: 9000,
